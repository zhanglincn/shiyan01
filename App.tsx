  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (gameState === GameState.PLAYING) {
        ...
        if (p.y > CANVAS_HEIGHT) { setGameState(GameState.GAME_OVER); }
      } 
    ...
    const drawRobotCat = (ctx: CanvasRenderingContext2D, p: Player) => {
      if (p.invincible % 10 < 5) {
        ...
        ctx.fillStyle = '#ffffff';
        ctx.beginPath();
        ctx.moveTo(-18, -32); ctx.lineTo(-20, -44); ctx.lineTo(-6, -32); ctx.fill(); ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(18, -32); ctx.lineTo(20, -44); ctx.lineTo(6, -32); ctx.fill(); ctx.stroke();

        // 方头
        ctx.fillRect(-22, -34, 44, 34);
        ctx.strokeRect(-22, -34, 44, 34);
        ...
        ctx.strokeRect(-4, -38, 8, 4);

        // '^ ^' 方形眼 (青色)
        ctx.strokeStyle = '#00e5ff';
        ctx.lineWidth = 2.5;
        // 左眼
        ctx.beginPath(); ctx.moveTo(-12, -18); ctx.lineTo(-9, -23); ctx.lineTo(-6, -18); ctx.stroke();
        // 右眼
        ctx.beginPath(); ctx.moveTo(6, -18); ctx.lineTo(9, -23); ctx.lineTo(12, -18); ctx.stroke();
    ...
    const drawMonster = (ctx: CanvasRenderingContext2D, obs: Obstacle, opacity: number = 1) => {
        ...
        // 橙色菌盖头 (类似板栗仔)
        ctx.fillStyle = '#d35400'; // 深橙色
        ctx.beginPath();
        // 底边
        ctx.moveTo(-16, 10 + wobble);
        ctx.lineTo(16, 10 + wobble);
        // 右侧斜向上
        ctx.lineTo(24, 0 + wobble);
        // 顶部弧线
        ctx.arcTo(0, -20 + wobble, -24, 0 + wobble, 20);
        // 左侧斜向下回到底边
        ctx.lineTo(-16, 10 + wobble);
        ctx.fill();
        ...
        ctx.fillStyle = '#ffffff';
        ctx.beginPath();
        ctx.ellipse(0, 15 + wobble, 10, 8, 0, 0, Math.PI * 2);
        ctx.fill();
        ctx.stroke();

        // 黑色愤怒眉毛 (连在一起的粗黑杠)
        ctx.fillStyle = '#000000';
        ctx.fillRect(-10, -8 + wobble, 20, 4);

        // 方形大白眼
        ctx.fillStyle = '#ffffff';
        ctx.fillRect(-8, -4 + wobble, 6, 8);
        ctx.fillRect(2, -4 + wobble, 6, 8);
        
        // 黑色小瞳孔
        ctx.fillStyle = '#000000';
        ctx.fillRect(-6, 0 + wobble, 3, 3);
        ctx.fillRect(4, 0 + wobble, 3, 3);

        // 黑色小脚
        ctx.fillStyle = '#000000';
        ctx.fillRect(-12, 22 + wobble, 8, 6);
        ctx.fillRect(4, 22 + wobble, 8, 6);
    ...
    const draw = () => {
      ...
      drawStaticRectLantern(ctx, CANVAS_WIDTH / 3, 60);
      drawStaticRectLantern(ctx, (CANVAS_WIDTH * 2) / 3, 60);

      for (let i = 0; i < 4; i++) {
        const cx = i * 260 + 100; const cy = 100 + Math.sin(i * 1.5) * 40;
        ctx.fillStyle = '#fff'; ctx.beginPath(); ctx.arc(cx, cy, 20, 0, Math.PI * 2); ctx.arc(cx + 25, cy - 8, 26, 0, Math.PI * 2); ctx.arc(cx + 45, cy + 2, 20, 0, Math.PI * 2); ctx.fill();
      }

      for (let i = 0; i < 4; i++) { drawHill(ctx, 80 + i * 240, groundYLevel); drawBush(ctx, 180 + i * 240, groundYLevel, 80); }
      ...
      drawRobotCat(ctx, playerRef.current);
      ctx.restore();
    };
      ...
      <div className="relative border-4 border-gray-800 shadow-2xl overflow-hidden rounded-lg">
          ...
          <div className="flex gap-20">
            <div className="flex items-center gap-2">
              <div className="flex flex-col">
                <span className="text-white text-[12px]">金币</span>
                <span className="text-yellow-400 text-[18px]">x{coins.toString().padStart(2, '0')}</span>
              </div>
            </div>
          </div>
        ...
        {gameState === GameState.GAME_OVER && (
          <div className="absolute inset-0 bg-black/90 flex flex-col items-center justify-center z-20">
            ...
            <p className="mb-8 text-gray-400">{time <= 0 ? "时间耗尽了！" : "注意脚下，不要掉下去！"}</p>
